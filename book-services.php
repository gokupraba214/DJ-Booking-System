...
$query->bindParam(':totalp',$totalp,PDO::PARAM_STR);
$query->bindParam(':addinfo',$addinfo,PDO::PARAM_STR);

 $query->execute();
   $LastInsertId=$dbh->lastInsertId();
   if ($LastInsertId>0) {
    echo '<script>alert("Your Booking Request Has Been Send. We Will Email You Soon.")</script>';
...
<html>
<head>
<title> DJ REVVY BOOKING SERVICE || Contact Us</title>
...
<option value="<?php echo htmlentities($row->EventType);?>"><?php echo htmlentities($row->EventType);?></option>
 <?php } ?>
						 ...
						 <ul>
							 <li class="text-info">Total People Attend</li>
							 <li><input type="text" class="text" name="totalp" required="true" maxlength="20"></li>
						 </ul>
						 <ul>
						 ...
						 <ul>
							 <li style="color:white" class="text">Booking Process will take 2 to 3 days. We will contact you soon.</li>
						 </ul>												 
					 </form>
				 </div>
				 <div class="col-md-6 contact-right">
					 	<div class="contact-map">
						<img src="images/tt.jpg" class="img-responsive" height="900" width="500" alt=""/>
						</div>
				 </div>
